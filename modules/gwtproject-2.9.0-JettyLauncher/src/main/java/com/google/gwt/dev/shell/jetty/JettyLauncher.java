...
public class JettyLauncher extends ServletContainerLauncher {
    ...
    private static final boolean USE_CUSTOM_RESOURCES_RESOLVER = true;
        ...
        public void log(Request request, Response response) {
            ...
            if (logger.isLoggable(logStatus)) {
                ...
                if (branch.isLoggable(logHeaders)) {
                    logHeaders(branch.branch(logHeaders, "Request headers"), logHeaders, request.getHttpFields());
                    logHeaders(branch.branch(logHeaders, "Response headers"), logHeaders, response.getHttpFields());
                }
            }
        }
    ...
    protected static class JettyServletContainer extends ServletContainer {
        ...
        public JettyServletContainer(TreeLogger logger,
                                     Server server,
                                     WebAppContext wac,
                                     int actualPort,
                                     File appRootDir) {
    ...
    /**
     * A {@link WebAppContext} tailored to GWT hosted mode. Features hot-reload with
     * a new {@link WebAppClassLoader} to pick up disk changes. The default Jetty
     * {@code WebAppContext} will create new instances of servlets, but it will not
     * create a brand new {@link ClassLoader}. By creating a new {@code
    ...
    protected static final class WebAppContextWithReload extends WebAppContext {
         ...
         * brought in dynamically from the system path. A warning is issued when this
         * occurs.
         */
        private class WebAppClassLoaderExtension extends WebAppClassLoader {
            ...
            private final ClasspathPattern systemClassesFromWebappFirst =
                new ClasspathPattern(new String[] { "-javax.servlet.", "-javax.el.", "javax.", });
            private final ClasspathPattern allowedFromSystemClassLoader =
                new ClasspathPattern(new String[] { "org.eclipse.jetty.", "javax.websocket.",
                        // Jasper
                        "org.apache.jasper.", "org.apache.juli.logging.", "org.apache.tomcat.", "org.apache.el.",
                        // Xerces
            ...
            public Enumeration<URL> getResources(String name) throws IOException {
                ...
                List<URL> fromParent = isServerClass(name) ? Collections.<URL>emptyList()
                ...
                Iterator<URL> fromWebapp = isSystemClass(name) && !fromParent.isEmpty()
                    ? Collections.<URL>emptyIterator() : Iterators.forEnumeration(findResources(name));

                if (USE_CUSTOM_RESOURCES_RESOLVER) {
                    // Custom handling
                    final Enumeration<URL> resources;
                    if (fromWebapp.hasNext()) {
                        resources = Iterators.asEnumeration(fromWebapp);
                    } else if (!fromParent.isEmpty()) {
                        resources = Iterators.asEnumeration(fromParent.iterator());
                    } else {
                        resources = Iterators.asEnumeration(Iterators.concat(fromWebapp, fromParent.iterator()));
                    }
                    return resources;
                }
            ...
            protected Class<?> findClass(String name) throws ClassNotFoundException {
                ...
                // Special-case JDBCUnloader; it should always be loaded in the webapp
                // classloader
                ...
                // Warn, add containing URL to our own ClassLoader, and retry the call.
                String warnMessage = "Server class '" + name +
                    "' could not be found in the web app, but was found on the system classpath";
            ...
            private boolean addContainingClassPathEntry(String warnMessage, URL resource, String resourceName) {
                ...
                branch = branch.branch(logLevel,
                                       "Adding classpath entry '" + classPathURL +
        ...
        /**
         * Parent ClassLoader for the Jetty web app, which can only load JVM classes. We
         * would just use <code>null</code> for the parent ClassLoader except this makes
         * Jetty unhappy.
         */
        ...
        /**
         * In the usual case of launching {@link com.google.gwt.dev.DevMode}, this will
         * always by the system app ClassLoader.
         */
            ...
            for (String arg : arguments.split(",")) {
                ...
                if ("ssl".equals(tag)) {
                    ...
                    logger.log(TreeLogger.ERROR,
                               "Unexpected argument to " + JettyLauncher.class.getSimpleName() + ": " + arg);
                    return false;
                }
            }
            if (useSsl) {
     ...
     * figure out a better way to do this for SCLs in general. Please do not depend
     * on this method, as it is subject to change.
     */
    ...
    public ServletContainer start(TreeLogger logger, int port, File appRootDir) throws Exception {
        ...
        try {
            ...
            logger.log(TreeLogger.Type.DEBUG,
                       "jetty-plus isn't on the classpath, JNDI won't work. This might also affect annotations scanning and JSP.");
        }
        try {
            ...
            logger.log(TreeLogger.Type.DEBUG,
                       "jetty-annotations isn't on the classpath, annotation scanning won't work. This might also affect annotations scanning.");
        }
        ...
        if (connector.getLocalPort() < 0) {
            branch.log(TreeLogger.ERROR,
                       String.format("Failed to connect to open channel with port %d (return value %d)",
                                     port,
                                     connectorPort));
        }
    ...
    /**
     * This is a modified version of JreMemoryLeakPreventionListener.java found in
    ...
    private void jreLeakPrevention(TreeLogger logger) {
         ...
         *
         * Those libraries / components known to trigger memory leaks due to eventual
         * calls to requestLatency(long) are: -
        ...
        /*
         * Creating a MessageDigest during web application startup initializes the Java
         * Cryptography Architecture. Under certain conditions this starts a Token
         * poller thread with TCCL equal to the web application class loader.
         *
        ...
        /*
         * Several components end up opening JarURLConnections without first disabling
         * caching. This effectively locks the file. Whilst more noticeable and harder
         * to ignore on Windows, it affects all operating systems.
         *
        ...
        /*
         * Haven't got to the root of what is going on with this leak but if a web app
         * is the first to make the calls below the web application class loader will be
         * pinned in memory.
         */
